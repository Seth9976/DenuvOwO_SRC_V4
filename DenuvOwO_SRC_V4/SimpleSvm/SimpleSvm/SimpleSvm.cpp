...
/*!
    ...
    @details    This function allocates page aligned nonpaged pool where backed
                ...
                filled and must be freed with SvFreeContiguousMemory.
...
// KUSER_SHARED_DATA spoofing related functions below.

UINT64
GetCr3ByProcessId(HANDLE ProcessId)
{
...
PT_ENTRY_64* GetPte(UINT64 virtual_address, uintptr_t pml4_base_pa)
{
    ...
    pdpt = (PDPTE_64*)PfnToVirtualAddr(pml4e->Fields.PageFrameNumber);
    ...
    if ((pdpte->Fields.LargePage == TRUE))
    {
        return (PT_ENTRY_64*)pdpte;
    }
    else if (pdpte->Fields.Present == FALSE)
    {
        return NULL;
    }
    ...
    pd = (PDE_64*)PfnToVirtualAddr(pdpte->Fields.PageFrameNumber);
    ...
    if (pde->Fields.LargePage == TRUE)
    {
        return (PT_ENTRY_64*)pde;
    }
    else if (pde->Fields.Present == FALSE)
    {
        return NULL;
    }
    ...
    pt = (PTE_64*)PfnToVirtualAddr(pde->Fields.PageFrameNumber);
    ...
    while (StopCounterThread == FALSE)
    {
        if (TargetProcessId)
        {
            ...
            else
            {
                ...
                TargetProcessId = NULL;
                TargetCR3 = NULL;
                SvDebugPrint("Failed to LookupProcessByProcessId, can not spoof KUSER only for the game process.");
                goto End;
            }
            ...
            if (NewKuserSharedData)
            {
                ...
                *(UINT8*)(NewKusdAddress + 0x290) = 0x0;    // Disable MONITORX support
                *(UINT8*)(NewKusdAddress + 0x294) = 0x0;    // Disable RDTSCP support
                *(UINT8*)(NewKusdAddress + 0x295) = 0x0;    // Disable RDPID support
                *(UINT8*)(NewKusdAddress + 0x297) = 0x0;    // Disable RDRAND support

                // XSAVE related stuff
                *(UINT8*)(NewKusdAddress + 0x285) = 0x0;    // Disable XSAVE support
                *(UINT8*)(NewKusdAddress + 0x29B) = 0x0;    // Disable AVX support
                *(UINT8*)(NewKusdAddress + 0x29C) = 0x0;    // Disable AVX2 support

                *(UINT64*)(NewKusdAddress + 0x3D8) = 0x0;   // EnabledFeatures
                *(UINT64*)(NewKusdAddress + 0x3E0) = 0x0;   // EnabledVolatileFeatures
                *(UINT32*)(NewKusdAddress + 0x3EC) = 0x0;   // ControlFlags
                memset((void*)(NewKusdAddress + 0x3F0), 0x00, 0x200);   // Features
                *(UINT64*)(NewKusdAddress + 0x5F0) = 0x0;   // EnabledSupervisorFeatures
                *(UINT64*)(NewKusdAddress + 0x5F8) = 0x0;   // AlignedFeatures
                memset((void*)(NewKusdAddress + 0x604), 0x00, 0x200);   // AllFeatures
                *(UINT64*)(NewKusdAddress + 0x808) = 0x0;   // EnabledUserVisibleSupervisorFeatures
                *(UINT64*)(NewKusdAddress + 0x810) = 0x0;   // ExtendedFeatureDisableFeatures
                //
                ...
                *(UINT32*)(NewKusdAddress + 0xFFC) = 0x13371337;    // Indicate that the game process can proceed execution to avoid race condition on snail PCs
            }

            if (!NotifyRoutineActive) {
                NTSTATUS RoutineStatus = PsSetCreateProcessNotifyRoutine(ProcessExitNotifyRoutine, FALSE);
                if (NT_SUCCESS(RoutineStatus)) {
...
// KUSER_SHARED_DATA spoofing related functions above.
    ...
    )
{
    ...
    if (VpData->GuestVmcb.StateSaveArea.Cpl == 0x3)
    {
        if (GuestContext->VpRegs->Rax == 0x69696969)
        {
            if (!TargetCR3)
            {
                UINT64 DirectoryTableBase = GetCurrentProcessCr3();

                if (VpData->GuestVmcb.StateSaveArea.Cr3 == DirectoryTableBase)
                {
        ...
        if (VpData->GuestVmcb.StateSaveArea.Rip >= 0x140000000 && VpData->GuestVmcb.StateSaveArea.Rip <= 0x170000000)
        {
            //SvDebugPrint("Cr3 %p --- TargetCr3 %p\n", reinterpret_cast<void*>(VpData->GuestVmcb.StateSaveArea.Cr3), reinterpret_cast<void*>(TargetCR3));
        }
        ...
        if (VpData->GuestVmcb.StateSaveArea.Cr3 == TargetCR3)
        {
            if (leaf == 0x1)
            {
                ...
                GuestContext->VpRegs->Rbx = 0x00100800; // APIC ID masked off
                GuestContext->VpRegs->Rcx = 0x7EF8320B & ~((1 << 12) | (1 << 25) | (1 << 26) | (1 << 27) | (1 << 28) | (1 << 29) | (1 << 30)); // Disabled FMA3, AES, XSAVE, OSXSAVE, AVX, F16C, RDRAND
            ...
            if (leaf == 0x80000002)
            {
                GuestContext->VpRegs->Rax = 'uneD';
                GuestContext->VpRegs->Rbx = 'OwOv';
                GuestContext->VpRegs->Rcx = 'UPC ';
                GuestContext->VpRegs->Rdx = '1 @ ';
                goto Exit;
            }

            if (leaf == 0x80000003)
            {
                GuestContext->VpRegs->Rax = ' 733';
                GuestContext->VpRegs->Rbx = 'zHG';
                GuestContext->VpRegs->Rcx = 0;
    ...
    //
    GuestContext->VpRegs->Rax = static_cast<UINT32>(registers[0]);
    GuestContext->VpRegs->Rbx = static_cast<UINT32>(registers[1]);
    GuestContext->VpRegs->Rcx = static_cast<UINT32>(registers[2]);
    GuestContext->VpRegs->Rdx = static_cast<UINT32>(registers[3]);
    ...
    )
{
    ...
    else if (msr == IA32_MSR_LSTAR)
    {
        ...
        {
            if (VpData->GuestVmcb.StateSaveArea.LStar != LstarHook)
            {
                value.QuadPart = VpData->GuestVmcb.StateSaveArea.LStar;
            }

            else
            {
                value.QuadPart = OrigLstar;
            }
...
)
{
    ...
    if (TargetCR3 && VpData->GuestVmcb.StateSaveArea.Cr3 == TargetCR3 && VpData->GuestVmcb.StateSaveArea.Cpl == 0x3)
    {
...
#pragma prefast(suppress : __WARNING_USE_OTHER_FUNCTION, "Unrecoverable path.")
    ...
    if (guestContext.ExitVm != FALSE)
    {
        ...
        if (OrigLstar) //Restore LStar to original.
        {
    ...
    )
{
    ...
    VpData->GuestVmcb.ControlArea.InterceptException |= SVM_InterceptException_DB; //For hiding the LSTAR hook from Windows Patchguard
    ...
    //
    //VpData->GuestVmcb.ControlArea.NpEnable |= SVM_NP_ENABLE_NP_ENABLE; // Disabled due to issues with MMIO accesses
    ...
    //VpData->GuestVmcb.StateSaveArea.GPat = __readmsr(IA32_MSR_PAT); // Guest PAT—only used if nested paging enabled

    if ((VpData->GuestVmcb.StateSaveArea.Cr4 & UMIP) == 0)
    {
        if (gdtr.Limit < 0x7F) // UMIP-enabled Windows provides 0x7F as the dummy GDT limit value for CPL 3 SGDT instruction execution.
        {
            VpData->GuestVmcb.ControlArea.InterceptMisc1 |= SVM_INTERCEPT_MISC1_GDTR_READ;
        }
    }
    ...
    VpData->GuestVmcb.StateSaveArea.LStar = LstarHook; //Activate the LStar hook.
...
#pragma prefast(suppress : __WARNING_MEMORY_LEAK, "Ownership is taken on success.")
    ...
    if (!OrigLstar) //Can be done per-core if needed.
    {
...
Exit:
    if (NT_SUCCESS(status))
    {
        NTSTATUS ThreadStatus = PsCreateSystemThread(&CounterThreadHandle, 0, 0, 0, 0, CounterUpdater, NULL);
