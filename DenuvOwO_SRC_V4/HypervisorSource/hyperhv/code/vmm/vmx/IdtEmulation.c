...
VOID
RestoreDescriptorTableExiting()
{
    UINT32 SecondaryProccessorBasedVmExecControls = 0;

    VmxVmread32P(VMCS_CTRL_SECONDARY_PROCESSOR_BASED_VM_EXECUTION_CONTROLS, &SecondaryProccessorBasedVmExecControls);

    SecondaryProccessorBasedVmExecControls |= IA32_VMX_PROCBASED_CTLS2_DESCRIPTOR_TABLE_EXITING_FLAG;

    VmxVmwrite32(VMCS_CTRL_SECONDARY_PROCESSOR_BASED_VM_EXECUTION_CONTROLS, SecondaryProccessorBasedVmExecControls);

    VmxVmwrite32(VMCS_CTRL_EXCEPTION_BITMAP, 0x2); // #DB only
}

VOID
InjectPageFaultForDescriptorTableStore(UINT64 PageFaultAddress, BOOLEAN IsPagePresent)
{
    ...
    __writecr2(PageFaultAddress); //"The MOV CR2 instruction does not check that address written to CR2 is canonical."
                                  ...
                                  _In_ VMEXIT_INTERRUPT_INFORMATION InterruptExit)
{
    ...
    case EXCEPTION_VECTOR_GENERAL_PROTECTION_FAULT:
        ...
        if (VCpu->TestNumber)
        {
            VmxVmwrite64(VMCS_GUEST_GDTR_LIMIT, VCpu->TestNumber);
            VCpu->TestNumber = 0x0;
        }
    ...
    case EXCEPTION_VECTOR_DEBUG_BREAKPOINT:
        ...
        if (InterruptExit.InterruptionType == 0x5) // Privileged software exception - INT1/ICEBP
        {
            UINT32 ExitInstrLength;

            VmxVmread32P(VMCS_VMEXIT_INSTRUCTION_LENGTH, &ExitInstrLength);
            VmxVmwrite32(VMCS_CTRL_VMENTRY_INSTRUCTION_LENGTH, ExitInstrLength);
        }

        if (VCpu->LastVmexitRip == LstarHook)
        {
            VmxVmwrite64(VMCS_GUEST_RIP, OrigLstar);
        }
        ...
        if (SecondaryProcBasedVmExecControls & IA32_VMX_PROCBASED_CTLS2_DESCRIPTOR_TABLE_EXITING_FLAG)
        {
            goto InjectDB;
        }
