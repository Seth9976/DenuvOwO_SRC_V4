...
UINT64 TargetCR3 = 0;
UINT64 OrigLstar = 0;
UINT64 TargetSysHandler = 0;
UINT64 SyscallBypassMagic = 0x1337133713371337;

BOOLEAN ProcessExitCleanup = 0x0;
PMDL KuserMDL = 0x0;
...
VOID CounterUpdater(PVOID Context)
{
    ...
    while (StopCounterThread == FALSE)
    {
        if (TargetProcessId)
        {
            ...
            CR3_TYPE TargetCr3 = { 0 };
            TargetCr3.Flags = LayoutGetCr3ByProcessId(TrackedProcessId).Flags;

            KAPC_STATE State = { 0 };

            KeStackAttachProcess(TargetProcess, &State);

            PPAGE_ENTRY TargetProcessKuserPte = MemoryMapperGetPteVaWithoutSwitchingByCr3(0x7FFE0000, PagingLevelPageTable, TargetCr3);
            ...
            if (NewKuserSharedData)
            {
                ...
                *(UINT8*)(NewKusdAddress + 0x290) = 0x0;    // Disable MONITORX usage
                *(UINT8*)(NewKusdAddress + 0x294) = 0x0;    // Disable RDTSCP usage
                *(UINT8*)(NewKusdAddress + 0x295) = 0x0;    // Disable RDPID usage
                *(UINT8*)(NewKusdAddress + 0x297) = 0x0;    // Disable RDRAND usage
...
VmxVirtualizeCurrentSystem(PVOID GuestStack)
{
    ...
    MsrHandleSetMsrBitmap(VCpu, IA32_LSTAR, 0x1, 0x1);

    HvSetExceptionBitmap(VCpu, EXCEPTION_VECTOR_DEBUG_BREAKPOINT);

    if (!OrigLstar)
    {
        OrigLstar = __readmsr(IA32_LSTAR);
    }

    __writemsr((unsigned long)IA32_LSTAR, LstarHook);
...
VmxSetupVmcs(VIRTUAL_MACHINE_STATE * VCpu, PVOID GuestStack)
{
    ...
    UINT64 GuestCR4 = __readcr4();

    UINT16 GdtLimit = AsmGetGdtLimit();

    if ((GuestCR4 & UMIP) != 0 || GdtLimit > 0x7F)
    {
    ...
    else
    {
        SecondaryProcBasedVmExecControls = HvAdjustControls(
            ...
            IA32_VMX_PROCBASED_CTLS2_ENABLE_INVPCID_FLAG |
            IA32_VMX_PROCBASED_CTLS2_ENABLE_XSAVES_FLAG |
            IA32_VMX_PROCBASED_CTLS2_DESCRIPTOR_TABLE_EXITING_FLAG |
            IA32_VMX_PROCBASED_CTLS2_ENABLE_USER_WAIT_PAUSE_FLAG,
            IA32_VMX_PROCBASED_CTLS2);
    }
...
VmxVmxoff(VIRTUAL_MACHINE_STATE * VCpu)
{
    ...
    __writemsr((unsigned long)IA32_LSTAR, OrigLstar);
